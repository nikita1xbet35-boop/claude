...
| Тесты (24 проверки) | ✅ `node partner-bots/test.mjs` |
| Токены в Cloudflare Secrets | ⛔ **нужен ты** — см. §1 |
| `setWebhook` на 6 ботов | ⛔ **нужен ты** — см. §3 |
| Адрес формы регистрации | ⛔ **нужен ты** — см. §4 |
| Тексты FAQ на EN/FR/UZ | ⛔ **нужен ты** — см. §5 |

Бот запускается и работает без двух последних пунктов: он честно говорит, что
ссылка ещё не настроена, и отдаёт контакт менеджера. Ничего не ломается и
никакая ложная ссылка не уходит.
...
Скрипт делает это для всех шести за раз:
