...
| Тесты (71 проверка) | ✅ `node partner-bots/test.mjs` |
...
| Ключи Cloudflare и токены всех восьми ботов в секретах **GitHub** | ✅ заведены |
...
| Сверка отметки сборки после деплоя | ✅ шаг «Проверить, что воркер видит базу» |
| Журнал событий пользователей `bot_user_events` + бэкфилл | ✅ миграция `056` |
| Админ-бот: роут `/admin-bot`, уведомления, `/today /week /stats /user /last` | ✅ `worker.js`, см. §3a |
| `ADMIN_BOT_TOKEN` и `ADMIN_CHAT_ID` в секретах GitHub | ⬜ **заводит Ник** |
...
| `ADMIN_BOT_TOKEN` | токен админ-бота из BotFather (уведомления о новых пользователях) |
| `ADMIN_CHAT_ID` | Telegram user_id Ника — единственный получатель. Узнать: написать @userinfobot |
...
`ADMIN_TZ_OFFSET` — не секрет, а обычная переменная в `wrangler.jsonc`: часовой
пояс, в котором админ-боту печатать время (по умолчанию `2`, то есть UTC+2).

Пара `ADMIN_*` необязательна. Без неё воркер деплоится как раньше, роут
`/admin-bot` молчит, а восемь партнёрских ботов работают в полном объёме:
отправка уведомления обёрнута в try/catch и её отсутствие для пользователя
незаметно. Но и Ник тогда не увидит ни одного нового лида, поэтому итоговый шаг
воркфлоу об этом предупреждает отдельной строкой.
...
# Админ-бот: токен и получатель уведомлений. Необязательны — без них молчит
# только он сам.
...
export ADMIN_BOT_TOKEN="..."   # необязательно: без него админ-бот пропускается
./set-webhooks.sh
```

Админ-бот регистрируется тем же скриптом, но на собственный роут `/admin-bot`
(не `/bot/{slug}`): он не входит во флот из восьми, у него нет ни ГЕО, ни языка,
и обращается к нему один человек.

---

## 3a. Админ-бот

Отдельного воркера у него нет — это тот же `partner-bots`, роут `/admin-bot`,
свой токен. Логика уведомления живёт вплотную к обработке `/start`, разносить её
по двум воркерам было бы нечем оправдать.

Что стоит сделать в BotFather после первого деплоя — `/setcommands`:

```
today - сводка за сегодня
week - сводка за 7 дней
stats - всё время, воронка и конверсии
user - карточка пользователя: /user @name или /user 123456789
last - последние N пользователей
help - список команд
```

Команды от любого chat_id, кроме `ADMIN_CHAT_ID`, игнорируются **молча** —
посторонний не должен по ответу бота понять, что бот существует.

Проверить, что дошло:

- `/help` в админ-боте — если молчит, скорее всего `ADMIN_CHAT_ID` не тот
  (в нём должен быть user_id Ника, а не имя и не id чата);
- `/start` в любом партнёрском боте с нового аккаунта — уведомление приходит
  за секунды. Со старого не придёт, и это правильно: уведомление только на
  первый `/start` в конкретном боте.
...
Файл лежит в `partner-bots/public/geo.pdf` и выкатывается вместе с воркером, а
адрес бот строит из origin входящего запроса — то есть из того самого адреса,
по которому Telegram до него достучался. Настраивать нечего.

Почему не иначе:

- **не к дашборду** — там пароль-гейт, Telegram получил бы форму входа вместо
  файла;
- **не через `raw.githubusercontent`** — это привязало бы работу бота к тому,
  что репозиторий останется публичным;
- **не собирать адрес из имени воркера** — поддомен `workers.dev` свой у
  каждого аккаунта, и угаданный адрес выглядит правдоподобно, ведя в никуда.

Если файл понадобится раздавать откуда-то ещё, `system_config.geo_pdf_url`
имеет приоритет:
...
Обновляя справочник, замени и сам PDF — он и таблица должны совпадать, иначе
бот скажет одно, а файл покажет другое.
