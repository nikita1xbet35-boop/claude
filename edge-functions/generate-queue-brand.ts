...
// full reasoning. This module ships paused (pipeline_limits.brand.paused),
// so today it's inert either way, but wiring it now means unpausing brand
// outreach for a second brand later doesn't ALSO require remembering to add
// this check at that point.
  ...
  try {
    ...
    if (error) {
      await quiet(supabase.from('error_log').insert([{
        level: 'warning', service: 'generate-queue-brand',
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: candidates, error: leadsErr } = await supabase.from('leads')
      .select('id, brand, brand_id, url, contact_email, fit_score, email_status, exclude_reason, '
