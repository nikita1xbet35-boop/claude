...
async function scheduledHandler(event, env, ctx) {
    ...
    // ── Планировщик ────────────────────────────────────────────────────────
    // У воркера здесь ровно одна обязанность: раз в минуту разбудить
    // диспетчера. Что запускать и как часто — решает он, читая scheduled_tasks.
    //
    // Сама логика диспетчера живёт в edge-функции, а не тут, и это не вкусовое
    // решение. Воркер ходит в базу под ANON-ключом, а диспетчер меняет
    // расписание: занимает блокировки, пишет статусы, отключает задачи. Выдать
    // такие права анонимному ключу, который лежит открытым текстом в
    // index.html, значило бы позволить любому остановить систему одним
    // запросом.
    //
    // Первая версия этой логики стояла здесь и не отработала ни одного тика:
    // scheduled_tasks закрыта для anon, первый же запрос отдавал 403, ошибку
    // глотал try/catch, и конвейер простоял 41 час, выглядя живым.
    await call('dispatch-tasks', {});
}
