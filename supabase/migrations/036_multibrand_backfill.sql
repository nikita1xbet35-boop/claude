...
-- ── п.4.1 ТЗ, последнее предложение ─────────────────────────────────────────
-- "если owned_until < now() — сразу считается свободным, next_eligible_brand_id
-- считается по 4.3." Многие из ~4678 контактов старше 3 дней уже сегодня, так
-- что без этого вызова часть новых строк ушла бы в реестр с "свободен, но
-- следующий бренд неизвестен" — то есть повторный запуск ротации нужен был бы
-- ДО первой попытки второго бренда его забрать, а не после.
SELECT public.fn_rotate_contacts();
...
-- edge-функция ещё не проставляет brand_id на каждой вставке — error_log/
-- api_usage/funnel_stats пишутся каждый прогон крона. Включить NOT NULL здесь
-- значит уронить каждую вставку в проде в течение 15 минут после деплоя этой
-- миграции, тем же способом, что убивал find-and-queue на отсутствующий
-- quiet(). NOT NULL накатывается отдельным шагом ПОСЛЕ того как edge-функции
-- начнут писать brand_id на каждую вставку (следующий блок работ).
